...
void overrideConfig(all_args_t &args, int argc, char* argv[]) {
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (std::strcmp(argv[i], "--frequency_change_rate") == 0 && i + 1 < argc) {
...
void writeCSV(const std::string &filename, const std::vector<std::complex<double>> &samples) {
...
int main(int argc, char* argv[]) {
    ...
    auto samples = generateComplexSineWave(args.amplitude,
                                           args.initial_frequency,
                                           args.frequency_change_rate,
                                           args.initial_phase,
                                           args.num_samples,
                                           args.sample_rate);
    ...
    if (args.write_iq) {
        ...
        std::cout << "IQ binary data written to " << args.output_iq_file << std::endl;
    }
